...
- Restricted-app install receipts, content-addressed package snapshots, per-automation enablement/cadence state and bounded run receipts, and Space-and-app-scoped JSON storage under the application-data `restricted-apps` directory, plus separately encrypted restricted-app connections in `restricted-app-connections.bin`.
