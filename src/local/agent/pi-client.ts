...
export function createRestrictedAppProposalTool(input: {
  ...
  return {
    ...
    description: "Submit a completed sandboxed app package inside the current Space for human review. Workspace inspects and hashes the folder itself. This creates a review proposal only: it does not run or install code, grant network, file, or notification access, enable background work, or store credentials.",
    ...
    promptGuidelines: [
      ...
      "The package must contain package.json with an agentApp path and already-built local assets; Workspace never runs npm or installs dependencies. agent-app.json version 1 has id, title, optional description, runtime {kind:'sandboxed-web',entry,worker?}, ui {icon?}, tools, optional background {intervalMinutes:15..1440}, and permissions {network,files,notifications?}. A notification is {id,title,description} with static single-line reviewed copy and requires a worker plus background schedule. A file permission is {id,target:'file'|'directory',access:'read'|'read-write'}. A network permission has id, target ({kind:'public-https',origin} or {kind:'loopback-http',host:'127.0.0.1'|'::1',port}), explicit GET/POST/PUT/PATCH/DELETE methods, and auth. Public auth supports none, api-key {header}, bearer, basic, or oauth2-pkce {issuer,clientId,scopes}; loopback is anonymous only. Never put a secret in the package.",
      ...
      "A declared worker is a browser ES module. Export handleAction(action,input) for tools and handleBackground(event) for a background schedule. Tool input/result schemas use the bounded closed JSON-Schema subset and object schemas set additionalProperties:false. Network, file, and storage powers exist only during an accepted worker operation. Notifications are narrower: only handleBackground may select a separately granted static category while background work is enabled. Treat optional powers as optional and catch denied notification or connection calls without failing unrelated work.",
      "Do not claim an app is installed when propose_space_app succeeds. It creates a digest-pinned review only; installation, each network/file/notification grant, connection setup, and background enablement remain separate human actions.",
    ],
    parameters: {
      type: "object",
      properties: {
    ...
    async execute(_toolCallId, params, signal) {
      ...
      const text = result.status === "pending" && result.proposal
        ? `Workspace inspected ${result.proposal.review.manifest.title} and opened a human review pinned to revision ${result.proposal.review.digest}. No code was executed or installed; no network, file, or notification access, credential, or background work was granted.`
        : "The app proposal was cancelled. No code was executed or installed; no network, file, or notification access, credential, or background work was granted.";
